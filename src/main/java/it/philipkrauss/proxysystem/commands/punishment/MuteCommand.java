...
public class MuteCommand implements SimpleCommand {
    ...
    private static final String PERMISSION = "proxy.command.mute";
    private static final String BYPASS_PERMISSION = "proxy.command.mute.bypass";
    private static final String ADMIN_PERMISSION = "proxy.command.mute.admin";
    ...
    public void execute(Invocation invocation) {
        ...
        if (args.length != 2) {
            source.sendMessage(Component.text(instance.getPunishPrefix() + "§7Verwende §8● §e/mute <Spieler> <Grund>"));
            List<PunishReason> reasons = punishProvider.getReasons().stream().filter(punishReason -> punishReason.getId() > 200 && punishReason.getId() <= 300).toList();
            if (reasons.size() == 0) {
            ...
            reasons.forEach(reason -> source.sendMessage(Component.text(
                    instance.getPunishPrefix() + "§8(§e" + (reason.getId() - 200) + "§8) §7" + reason.getReason() + " §8● §e" + reason.getDurationText())));
            return;
        }
        ...
        PunishReason punishReason = punishProvider.getReason(reasonId + 200);

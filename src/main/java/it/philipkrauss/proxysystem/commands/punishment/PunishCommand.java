...
public class PunishCommand implements SimpleCommand {
    ...
    private static final String PERMISSION = "proxy.command.punish";
    private static final String BYPASS_PERMISSION = "proxy.command.punish.bypass";
    private static final String ADMIN_PERMISSION = "proxy.command.punish.admin";
    ...
    public void execute(Invocation invocation) {
        ...
        if (args.length != 2) {
            source.sendMessage(Component.text(instance.getPunishPrefix() + "§7Verwende §8● §e/punish <Spieler> <Grund>"));
            List<PunishReason> reasons = punishProvider.getReasons().stream().filter(punishReason -> punishReason.getId() <= 50).toList();
            if (reasons.size() == 0) {
            ...
            reasons.forEach(reason -> source.sendMessage(Component.text(
                    instance.getPunishPrefix() + "§8(§e" + reason.getId() + "§8) §7" + reason.getReason() + " §8● §e" + reason.getDurationText())));
            return;
        }
